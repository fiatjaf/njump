...
func relayLastNotes(ctx context.Context, relayUrl string, limit int) iter.Seq[*nostr.Event] {
	...
	return func(yield func(*nostr.Event) bool) {
		for id := range internal.getEventsInRelay(relayUrl) {
		...
		if relay, err := sys.Pool.EnsureRelay(relayUrl); err == nil {
			ch, _ := relay.QueryEvents(ctx, nostr.Filter{
				Kinds: []int{1},
				Limit: limit,
			})
			for evt := range ch {
				sys.StoreRelay.Publish(ctx, *evt)
				internal.attachRelaysToEvent(evt.ID, relayUrl)
			}
		}
	}
}
