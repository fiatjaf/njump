			...
			"events-in-relay": {
				...
				Emit: func(t leafdb.DataType, value proto.Message, emit func(idxkey []byte, t leafdb.DataType, value proto.Message)) {
					...
					for _, r := range ee.Relays {
						emit([]byte(r), TypeEventInRelay, &EventInRelay{Id: ee.Id})
					}
				},
			},
		},
...
func (internal *InternalDB) scheduleEventExpiration(eventId string) {
	...
	if err := internal.UpdateQuery(leafdb.PrefixQuery("cached-id", idxkey), func(t leafdb.DataType, data proto.Message) (proto.Message, error) {
		ee := data.(*CachedEvent)
		ee.Expiry = time.Now().Add(time.Hour * 24 * 7).Unix()
		return ee, nil
	}); err != nil {
		log.Fatal().Err(err).Msg("failed to update schedule expirations")
	}
}
...
func (internal *InternalDB) getEventsInRelay(url string) iter.Seq[string] {
	return func(yield func(string) bool) {
		for value := range internal.DB.View(leafdb.ExactQuery("events-in-relay", []byte(url))) {
